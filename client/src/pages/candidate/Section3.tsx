...
export const Section3: React.FC = () => {
  ...
  const [prompt, setPrompt] = useState<Section3Prompt | null>(null);
  ...
  useEffect(() => {
    const fetchS3Data = async () => {
      try {
        ...
        setPrompt(s3Res.data.prompt);

        if (s3Res.data.response?.answerText) {
  ...
  return (
    <div className="bg-surface-canvas font-body-default text-text-primary antialiased min-h-screen flex flex-col justify-between">
      <CandidateNavbar
        timerNode={
          <ExamTimer
      ...
      <main className="w-full pt-8 pb-24 bg-surface-canvas flex-grow flex flex-col justify-between">
        <div className="flex flex-col w-full">
          <div className="w-full max-w-[680px] mx-auto px-unit-4 py-unit-6 flex flex-col gap-unit-6">
            <div className="text-text-muted font-body-default text-body-default">
              Section 3: Email Writing
            </div>

            <div className="rounded bg-surface-card p-unit-5 border border-border-rule flex flex-col gap-unit-4 shadow-sm">
              <p className="font-body-reading text-body-reading text-text-primary leading-relaxed">
                <strong className="font-semibold">Scenario: </strong>
                {scenarioLine.replace(/^Scenario:\s*/i, '')}
              </p>

              {requirementLines.length > 0 && (
                <div className="text-text-primary font-body-default text-body-default flex flex-col gap-unit-1">
            ...
            <div className="flex flex-col gap-unit-4">
              <textarea
                ...
                autoFocus
                className="w-full p-unit-4 rounded border border-border-rule bg-surface-card font-body-reading text-body-reading text-text-primary leading-relaxed resize-y focus:outline-none focus:border-text-muted transition-colors shadow-sm"
              />

              <div className="flex items-center justify-between">
                <div className="text-text-muted font-body-default text-body-default" id="word-badge">
                ...
                <button
                  ...
                  className="px-unit-5 py-unit-2 rounded bg-primary-container text-white font-label-prominent text-label-prominent hover:bg-[#172554] transition-colors cursor-pointer disabled:opacity-50"
                >
      ...
      <footer className="fixed bottom-0 left-0 right-0 w-full bg-surface-card border-t border-border-rule py-unit-4 z-40">
